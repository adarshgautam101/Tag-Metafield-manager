async function fetchTagsPage(admin: any, objectType: string, cursor = null) {
  ...
  if (objectType === "customer") {
    ...
    return {
      tags: customers.flatMap((c: any) => c.tags || []),
  ...
  if (objectType === "order") {
    ...
    return {
      tags: orders.flatMap((o: any) => o.tags || []),
  ...
  if (objectType === "article") {
    ...
    return {
      tags: articles.flatMap((a: any) => a.tags || []),
...
export async function handleFetch(admin: any, formData: any) {
  try {
...
export async function handleRemoveFromAll(admin: any, formData: any) {
  try {
    const objectType = formData.get("objectType");
    ...
    for (const item of items) {
      ...
      const tagsToRemove = tags.filter((t: string) => existing.includes(t));
      const missingTags = tags.filter((t: string) => !existing.includes(t));
...
export async function handleRemoveSpecific(admin: any, formData: any) {
  const tags = JSON.parse(formData.get("tags") || []);
  ...
  try {
    ...
    const tagsToRemove = tags.filter((t: string) => existingTags.includes(t));
    const missingTags = tags.filter((t: string) => !existingTags.includes(t));
...
export async function fetchResourceId(admin: any, resourceType: string, value: any) {
  const queries: any = {
    customer: {
    ...
    productVariants(first: 1, query: $value) {
      ...
      path: (res: any) => res?.productVariants?.edges?.[0]?.node?.product?.id,
    },
  };
